...
def propagate(nnf, feat_A, feat_AP, feat_B, feat_BP, patch_size, iters=2, rand_search_radius=200):
    ...
    if setting.is_debug_mode: print("\tElapse NND LOOP: " + str(datetime.timedelta(seconds=time.time() - start_time))[:-7])

    from queue import Queue
    q = Queue(A_size[1] * A_size[0])
    ...
    for i in range(iters):
        ...
        pixelmatch(q, ax_start, ay_start,
                    1,
                    nnf, nnd,
                    A_size, B_size,
                    feat_A, feat_AP,
                    ...
                    patch_size,
                    rand_search_radius)
        if setting.is_debug_mode: print("\tElapse PIXELMATCH: " + str(datetime.timedelta(seconds=time.time() - start_time_2))[:-7])
...
def pixelmatch(q, ax_start, ay_start, cpus, nnf, nnd, A_size, B_size, feat_A, feat_AP, feat_B, feat_BP, patch_size,
    ...
    for ay in y_idxs:
        ...
        for ax in x_idxs:
            ...
            if setting.is_first_frame or setting.currlayer_global<4 or setting.is_prev_frame_changed[ay, ax]:
                for jump in [8, 4, 2, 1]:
                    # left
                ...
                while_times.append(datetime.timedelta(seconds=time.time() - start_time).total_seconds())

            q.put([ax, ay, xbest, ybest, dbest])

    if setting.is_debug_mode: print("\tElapse WHILE TIME: " + str(datetime.timedelta(seconds=np.sum(while_times)))[:-7])
